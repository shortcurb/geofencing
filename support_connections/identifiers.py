...
#        print(json.dumps(vehicle_device_dict,indent=2))
        ...
        """
        {   # This, but not a dictionary
            "vin": "3C6TRVCG8LE131320",
            "fleetnumber": "LA451",
            "year": 2020,
            "model": "Dodge 2500 Promaster HiRo 136WB",
            "status": "A1 - Active and Rentable",
            "decomstatus": "DEC1.1 - FVIP Requested Decommission - MD:  ",
            "market": "Los Angeles",
            "ismi": "310170848506944",
            "imei": "862831033854154",
            "iccid": "89011703278485069440"
        }
        """
...
class Identify:
    ...
    def __init__(self):
        self.query_base = """
            SELECT v.vin, v.fleetnumber, v.year, v.model, v.status, v.decomstatus, v.market, d.ismi, d.imei, d.iccid 
            FROM vehicles v INNER JOIN devices d ON v.ismi=d.ismi
            WHERE
            """

    def lookup_known_ID(self, id, id_type, db: Optional[Database] = None,):
        if db == None:
            db = Database()
        ...
        if id_type in types.keys():
            query = self.query_base + types[id_type]+'.'+id_type + ' = ?'
            db_ids = db.execute_query(query,[str(id)])
            return [Vehicle_Device(item) for item in db_ids]

        else:
            return "I don't know how to look that id_type up"
