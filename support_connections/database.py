...
class GeoDB:
    ...
    def insert_gdf(self,tablename:str, groupname: str ,name: str, polygon):

        polygon['geom_wkt'] = polygon['geometry'].apply(lambda x: x.wkt)

        with self.engine.connect() as conn:
            with conn.begin():
                for index, row in polygon.iterrows():
                    insert_query = text(f"""
                    INSERT INTO {self.database}.{tablename} (groupname, name, geom)
                    VALUES (:groupname, :name, ST_GeomFromText(:geom_wkt)) ON DUPLICATE KEY UPDATE geom=VALUES(geom), groupname=VALUES(groupname)
                    """)
                    params = {
                        'groupname':groupname,
                        'name': name,
    ...
    def select_gdf(self, query: str, data: Optional[dict] = None) -> gpd.GeoDataFrame:
        ...
        if matches != []:
            for match in matches:
                query = query.replace(match, ", ST_AsText(geom) as geom")
        if data:
