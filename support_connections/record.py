...
class RecordFetcher:
    ...
    def _get_one_page_historical_records(self, ismi:int, start:datetime, end:datetime, page_number:int) -> List[Any]:
        ...
        """
        start_str = datetime.datetime.strftime(start,'%m/%d/%y %H:%M:%S')
        end_str = datetime.datetime.strftime(end,'%m/%d/%y %H:%M:%S')
...
class FluidFleetRecords:
    def __init__(self):
        ...
        self.translation = {
            'lat':'coordinates_x',
            'lng':'coordinates_y',
            'odometer_LVCAN':'odometer',
            'last-data-event':'transmit_date',
            'vehicle_battery_level':'event_data_IoItems_External_Power_Voltage',
            'device_battery_level':'battery_health',
            'OverSpeeding':'over_speeding'
        }
        self.ff = FluidFleet()

    def parse_ff_record(self, record_dict: Dict[str, Any]) -> 'Record':
        for key,value in self.translation.items():
            try:
                record_dict.update({value:record_dict[key]})
                record_dict.pop(key)
            except KeyError:
                pass
        return Record(record_dict)
    ...
    def get_and_parse_ff_records(self,start:Union[datetime.datetime,str], end:Union[datetime.datetime,str], offset:Optional[int] = 0, limit:Optional[int] = 10000):
        # You'll need to add pagination at some point
        if isinstance(start, datetime.datetime):
            start = datetime.datetime.strftime(start,'%Y-%m-%dT%H:%M:%S.00Z')
        if isinstance(end,datetime.datetime):
            end = datetime.datetime.strftime(end,'%Y-%m-%dT%H:%M:%S.00Z')
        ...
        r = self.ff.req_fluid_fleet(args)
        return [self.parse_ff_record(record) for record in r.json()['data']]
...
#    print(lr)
    stop = datetime.now()
    start = stop-datetime.timedelta(hours=12)
...
'''
Fluid Fleet Record looks like this:
    {
      "ismi": 310170845429887,
      "lat": "34.2143",
      "lng": "-83.89281",
      "odometer_LVCAN": 57147.02921694608,
      "event_date": "07-02-2024 20:08:36",
      "last-data-event": "07-02-2024 20:08:47",
      "vehicle_battery_level": 12805,
      "device_battery_level": "3986",
      "fuel_level": 41,
      "vehicle_type": 1,
      "OverSpeeding": 0,
      "reliability_status": ""
...
But its translated to be like a command center record
